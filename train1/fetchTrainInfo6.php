<?php
    $conn = new mysqli('localhost', 'root', 'ddreb@0660', 'db_train');
    if($conn === false){
    ...
    $sourceStationIdQuery = "SELECT `id` FROM `station_table` WHERE `stationCode`='$sourceCode';";
    $destinationStationIdQuery = "SELECT `id` FROM `station_table` WHERE `stationCode`='$destinationCode';";
    ...
    $test_qry1 = "SELECT `trainId` FROM `route_table_new` WHERE `stationId`= $sourceStationId ORDER BY arrival ASC";
    $test_qry2 = "SELECT `trainId` FROM `route_table_new` WHERE `stationId`= $destinationStationId ORDER BY arrival ASC";
    ...
    $weekDayWieseMatchedIds = array();
    foreach($matchedIds as $d){
        $trainIdFindQuery = "SELECT $weekDay FROM `train_table2` where `id`=$d;";
        $result8 = $conn->query($trainIdFindQuery);
        $row7 = $result8->fetch_array();
        if($row7["$weekDay"]=='1'){
            array_push($weekDayWieseMatchedIds, $d);
        }
    }


    // COUNTING THE NUMBER OF TRAINS
    $trainsCount = sizeof($weekDayWieseMatchedIds);
    ...
    for($i=0; $i<$trainsCount; $i++){
        $scheduleInfoQry = "SELECT `stationId`, `arrival`, `departure` FROM `route_table_new` WHERE `trainId`=$weekDayWieseMatchedIds[$i];";
        $scheduleInfoResult = $conn->query($scheduleInfoQry);
        $trainId = $weekDayWieseMatchedIds[$i]; // STORE TRAIN ID
