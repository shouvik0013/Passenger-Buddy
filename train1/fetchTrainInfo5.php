<?php
    ...
    $test_qry1 = "SELECT `trainId` FROM `route_table2` WHERE `stationId`= $sourceStationId ORDER BY arrival ASC";
    $test_qry2 = "SELECT `trainId` FROM `route_table2` WHERE `stationId`= $destinationStationId ORDER BY arrival ASC";
    ...
    for($i=0; $i<$trainsCount; $i++){
        $scheduleInfoQry = "SELECT `stationId`, `arrival`, `departure` FROM `route_table2` WHERE `trainId`=$matchedIds[$i];";
